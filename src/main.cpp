    ...
    while (true) {
        ...
        if(CO != NULL && CO->CANmodule[0]->CANnormal) {
            ...
            if (new_dmx_sig) {
                ...
                map_DMX16(raw_speed, &cmd_speed, WDY_MIN_SPEED, WDY_MAX_SPEED);
                map_DMX16(raw_position, &cmd_position, WDY_MIN_POSITION, WDY_MAX_POSITION);
                map_DMXcommand(raw_command, &cmd_command);

                map_DMX8(raw_accel, &cmd_accel, WDY_MIN_ACCEL, WDY_MAX_ACCEL);
                map_DMX8(raw_decel, &cmd_decel, WDY_MIN_DECEL, WDY_MAX_DECEL);
